...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float f, boolean flag, int i, int j)
    {
        ...
        {
            fontrenderer.drawStringWithShadow((new StringBuilder()).append("Minecraft Beta 1.1_02 (").append(mc.debug).append(")").toString(), 2, 2, 0xffffff);
        ...
        for(Cheat c : Client.i.cheatList.cheats) {
        	if(c.isToggled()) {
        		mods.add(c);
        	}
        }
        
        mods.sort((m2, m1) ->fontrenderer.getStringWidth(m1.getName()) - fontrenderer.getStringWidth(m2.getName()));
        
        
        int y = 0;
        for(Cheat c : mods) {
        	fontrenderer.drawStringWithShadow(c.getName(), (scaledresolution.getScaledWidth() * 2) - 
        			fontrenderer.getStringWidth(c.getName()), y, -1);
